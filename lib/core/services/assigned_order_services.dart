...
class AssignedOrderServices{
  getAssignedOrderList(Map<String,dynamic> body)async{
    String query = r"""query($shopAssistantId:ID!){
  assignedOrdersList(shopAssistantId:$shopAssistantId){
   id
    orderNumber
    products{
      id
    ...
    if (!result.haserror){
      ...
      if(result.data!=null){
        result.data["assignedOrdersList"].forEach((order)=>{
